...
$tipo       = $_FILES['dataCliente_cra']['type'];
$tamanio    = $_FILES['dataCliente_cra']['size'];
$archivotmp = $_FILES['dataCliente_cra']['tmp_name'];
...
foreach ($lineas as $linea) {
    ...
    if ($i != 0) {
        ...
        $fecha_proceso        = $datos[11] ? $datos[11]: '';
        $monto_usd            = $datos[12] ? $datos[12]: '';
        $monto                = $datos[13] ? $datos[13]: '';
        $tipo_entr_tarjeta    = $datos[14] ? $datos[14]: '';
        $riesgo_pais          = $datos[15] ? $datos[15]: '';
        $fecha_transaccion    = $datos[16] ? $datos[16]: '';
        $empresa              = $datos[17] ? $datos[17]: '';
        $cuenta_equivalente   = $datos[18] ? $datos[18]: '';
        $pais                 = $datos[19] ? $datos[19]: '';
        $moneda_origen        = $datos[20] ? $datos[20]: '';
        $monto_origen         = $datos[21] ? $datos[21]: '';
        $moneda_destino       = $datos[22] ? $datos[22]: '';
        $monto_destino        = $datos[23] ? $datos[23]: '';	

        if( !empty($plastico) ){
            $duplicidad = ("SELECT autorizacion,monto  FROM trx_incoming_cra WHERE autorizacion ='".($autorizacion)."' and  monto ='".($monto)."'");
        ...
        if ( $cant_duplicidad == 0 ) { 
            ...
            $insertarData = "INSERT INTO trx_incoming_cra( 
            moneda, tipo_transaccion, afiliado, plastico, 
            nombre_cliente, usuario, autorizacion, cod_concepto, 
            comercio, ciudad, cod_pais, fecha_proceso, 
            monto_usd,monto, tipo_entr_tarjeta, riesgo_pais, 
            fecha_transaccion, empresa, cuenta_equivalente, pais, 
                    ...
                    <div class="col-lg-12">
                        <div class="card w-50">
                            ...
                            <div class="card-footer text-muted">
                                Bitacoras de transacciones de reportes de PROFIT-COSTA RICA
                            </div>
                        </div>
                    </div>
                </div>
