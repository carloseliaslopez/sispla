...
$tipo       = $_FILES['dataCliente_gtm']['type'];
$tamanio    = $_FILES['dataCliente_gtm']['size'];
$archivotmp = $_FILES['dataCliente_gtm']['tmp_name'];
...
foreach ($lineas as $linea) {
    ...
    if ($i != 0) {
        ...
        if( !empty($plastico) ){
            $duplicidad = ("SELECT autorizacion,monto  FROM trx_incoming_gtm WHERE autorizacion ='".($autorizacion)."' and  monto ='".($monto)."'");
        ...
        if ( $cant_duplicidad == 0 ) { 
            ...
            $insertarData = "INSERT INTO trx_incoming_gtm( 
            moneda, tipo_transaccion, afiliado, plastico, 
            nombre_cliente, usuario, autorizacion,cod_concepto, 
            comercio, ciudad, cod_pais,fecha_proceso,monto_usd,
            monto,tipo_entr_tarjeta,riesgo_pais ,fecha_transaccion,pais
            ) VALUES(
                '$moneda','$tipo_transaccion','$afiliado','$plastico',
                '$nombre_cliente','$usuario','$autorizacion','$cod_concepto',
                '$comercio','$ciudad','$cod_pais','$fecha_proceso','$monto_usd',
                    ...
                    <div class="col-lg-12">
                        <div class="card w-50">
                            ...
                            <div class="card-footer text-muted">
                                Bitacoras de transacciones de reportes de incoming GUATEMALA-PROFIT
                            </div>
                        </div>
                    </div>
                </div>
